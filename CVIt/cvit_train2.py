...
def train_gpu(model, criterion, optimizer, scheduler, num_epochs, min_val_loss):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'validation']:
            ...
            if epoch%5==0:
                ...
                with open(f'./weight/weights_{epoch}.pkl', 'wb') as f:
                    pickle.dump([train_loss, train_accu, val_loss, val_accu], f)
                state = {'epoch': num_epochs+1,
                         'state_dict': model.state_dict(),
                         'optimizer': optimizer.state_dict(),
                         'min_loss':epoch_loss}
                torch.save(state,f'./weight/weights_{epoch}.pth')
                test(model)
    ...
    plt.savefig( 'accu_CViT_ffDeepfakes.png' )
    ...
    plt.xlabel('epoch')
    plt.legend(['Train', 'Validation'], loc='upper left')
    plt.savefig( 'loss_CViT_ffDeepfakes.png' )
